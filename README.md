...
node tests/tuition-window.mjs       # rail, offers, logic, and the docs/ copy name the same windows
...
`tests/tuition-window.mjs` is the drift check across everything that names a tuition window: the printed rail, the JSON-LD offers, the `windowFor()` logic (whose two boundaries it *derives* by walking the year rather than restating them), and every working document in `docs/`. Each doc must quote all three prices and date ranges exactly as the page prints them, and may use **no** dollar amount the page does not print — the legitimate set is read from `index.html`, so the savings figures beside the prices are allowed and an invented number is not. `docs/enrollment-push.md` gets one more check on top: it declares the window it was written for, and while it is marked `active` that window must still be the live one, so send-ready copy quoting a closed price fails the suite the morning the window flips. It also fixes the no-JavaScript contract — the shipped HTML must carry no `is-current` class and every "Current payment window" marker must ship `hidden`, so a stale window can never be baked into the file. Pass a date to rehearse a boundary; with no argument it uses today in America/New_York:
...
`docs/enrollment-push.md` is the outbound half: the copy that goes out before a tuition window closes. It carries a send calendar, five assets (one-to-one email, broadcast email, LinkedIn post, short direct message, last-call), an explicit list of what the push may never claim — no seat count, no countdown, no testimonial, no university implication, all of which the site itself refuses — and the two gates that come before any send, the undecided payment method and the fact that both boundary dates fall on a Saturday. It is written for one window at a time and declares which; the suite fails once that window closes, so the file cannot quietly become a set of sendable emails quoting last month's price.
